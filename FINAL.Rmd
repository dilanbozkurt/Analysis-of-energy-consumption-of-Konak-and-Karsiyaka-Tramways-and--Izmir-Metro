...
data1$year <- as.numeric(as.integer(data1$year))  # Convert one variable to numeric
data1$month <- as.numeric(as.integer(data1$month))  # Convert one variable to numeric
lapply(data1, class)
data1
```
...
data2$year <- as.numeric(as.integer(data2$year))  # Convert one variable to numeric
data2$month <- as.numeric(as.integer(data2$month))  # Convert one variable to numeric
lapply(data2, class)
data2
```
...
I want to see if the mean of consumption per passenger(i refer it as PER in this section) is significantly different or not between Konak and Karsiyaka tram since both of them are the same transportation type. Firstly, data3 dataframe holds the PER values.I created a data frame called "tramways" which has 2 columns named District that has Konak and Karsiyaka, other column named PER which has PER values. 
...
## CORRELATIONS AT KARSIYAKA TRAM
...
```{r}
#Remember cer values are from data1, tep from data2, per from data3.
shapiro.test(data1$karsiyaka.tram)
shapiro.test(data2$karsiyaka.tram)
shapiro.test(data3$karsiyaka.tram)
karsiyaka
```
...
karsiyaka.corr1 <- cor.test(data2$karsiyaka.tram, data3$karsiyaka.tram, method = "pearson")
karsiyaka.corr1

#h0 : There are no correlation between tep and cer at karsiyaka.tram
karsiyaka.corr2 <- cor.test(data2$karsiyaka.tram, data1$karsiyaka.tram, method = "pearson")
karsiyaka.corr2

#h0 : There are no correlation between cer and per passenger at karsiyaka.tram
karsiyaka.corr3 <- cor.test(data1$karsiyaka.tram, data3$karsiyaka.tram, method = "pearson")
karsiyaka.corr3
```
...
```{r}
shapiro.test(data1$konak.tram)
shapiro.test(data2$konak.tram)
shapiro.test(data3$konak.tram)
```
For the first two test, p- values are grater than 0.05 so null hypothesis is not rejected, for per passenger, p-value is less than 0.05 so the null hypothesis that the data are normally distributed is rejected. But I will assume normality for all of them.
```{r}
#h0 : There are no correlation between tep and per passenger at konak.tram
konak.corr1 <- cor.test(data2$konak.tram, data3$konak.tram, method = "pearson")
konak.corr1

#h0 : There are no correlation between tep and cer at konak.tram
konak.corr2 <- cor.test(data2$konak.tram,data1$konak, method = "pearson")
konak.corr2

#h0 : There are no correlation between cer and per passenger at konak.tram
konak.corr3 <- cor.test(data1$konak, data3$konak.tram, method = "pearson")
konak.corr3
```
...
metro.corr1 <- cor.test(data2$metro,data3$metro, method = "pearson")
metro.corr1

#h0 : There are no correlation between tep and cer at metro
metro.corr2 <- cor.test(data2$metro, data1$metro, method = "pearson")
metro.corr2

#h0 : There are no correlation between cer and per passenger at metro
metro.corr3 <- cor.test(data1$metro,data3$metro, method = "pearson")
metro.corr3
```
...
Cor.coeff = 0.46
p-values > 0.05
I can conclude that tep and cer are not significantly correlated for metro.I fail to reject null hypothesis.
...
1.b)Since the value of per passenger get by divided the number of people to consumed energy(petrol in our case),mathematically it is correct.

2)For all the correlation tests, there are no significant correlation between cer and others(tep and per passenger). But there should be a significant correlation between cer and tep since unit of tep is (CER-AUX-W/S). I think that is a type 2 error because I fail reject the null hypothesis when it is actually false.
...
linear1 = lm(data3$karsiyaka.tram ~ data2$karsiyaka.tram)
summary(linear1) 
```
...
plot(x=data2$karsiyaka.tram, y=data3$karsiyaka.tram ,pch=16 ,col = "purple") #Plot the results
...
```{r}
karsiyaka <- cbind(data2,data3)
colnames(karsiyaka)<-c("year","month","metro.tep","karsiyaka.tram.tep","konak.tram.tep","year","month","metro.per","karsiyaka.tram.per","konak.tram.per")
karsiyaka = karsiyaka[c("karsiyaka.tram.tep","karsiyaka.tram.per")]
karsiyaka
class(karsiyaka)
```
...
polyReg <- lm(karsiyaka$karsiyaka.tram.per ~ poly(karsiyaka$karsiyaka.tram.tep, 6, raw = TRUE), data = train.data)
summary(polyReg)
...
```{r}
# Build the model
model <- lm(train.data$karsiyaka.tram.per ~ poly(train.data$karsiyaka.tram.tep, 3, raw = TRUE), data = train.data)
model
```

```{r}
ggplot(train.data, aes(karsiyaka.tram.tep, karsiyaka.tram.per) ) +geom_point() + stat_smooth(method = lm, formula = y ~ poly(x, 3, raw = TRUE))
```
...
```{r}
linear1 = lm(data3$konak.tram ~ data2$konak.tram)
summary(linear1) 
```
At the summary, I can see the values of the intercept (“a") and the slope (“b”) for the tep.
a= 2.31
...
```{r}
mean(data2$konak.tram)
mean(data3$konak.tram)
```
...
```{r}
linear1 = lm(data3$metro ~ data2$metro)
summary(linear1) 
```
At the summary, I can see the values of the intercept (“a") and the slope (“b”) for the tep.
a= 2.69
